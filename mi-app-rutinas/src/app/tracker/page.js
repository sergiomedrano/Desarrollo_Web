...
function TrackerContenido() {
  ...
  // EFECTO PARA EL TIEMPO TOTAL (AHORA INCLUYE MOVILIDAD)
  useEffect(() => {
    // Corre siempre desde el segundo 1
    const intervaloTotal = setInterval(() => {
      setTiempoTotal(t => t + 1);
    }, 1000);
    return () => clearInterval(intervaloTotal);
  }, []);
      ...
      <div className="min-h-screen bg-gray-900 text-white flex flex-col font-sans p-6 animate-fade-in relative">
        <header className="flex justify-between items-center w-full mt-2">
          <div className="text-xs font-bold text-gray-500 flex items-center gap-2 bg-gray-800 px-3 py-2 rounded-xl border border-gray-700">
            ...
            <span className="font-mono text-emerald-400">{formatoTiempoDinamico(tiempoTotal)}</span>
          </div>
          <button onClick={() => router.push('/dashboard')} className="text-gray-500 font-bold text-xl hover:text-white">✕</button>
        </header>
        ...
        <div className="flex-1 space-y-4">
          {listaMovilidad.map((item, index) => (
            <div key={index} onClick={() => toggleCheckMovilidad(index)} className={`flex items-center gap-4 p-5 rounded-2xl border-2 cursor-pointer transition-all ${checksMovilidad[index] ? 'bg-emerald-500/10 border-emerald-500 shadow-lg shadow-emerald-500/10' : 'bg-gray-800 border-gray-700 hover:border-gray-500'}`}>
              <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center transition-colors ${checksMovilidad[index] ? 'bg-emerald-500 border-emerald-500 text-gray-900' : 'border-gray-500 text-transparent'}`}><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"></path></svg></div>
        ...
        <div className="mt-8 space-y-4">
          ...
          <button onClick={() => setFaseActiva("entrenamiento")} className="w-full text-center text-gray-500 text-sm font-bold hover:text-gray-400 py-2 transition-colors">Saltar calentamiento (No recomendado)</button>
        </div>
      </div>
    );
  }
      ...
      <header className="sticky top-0 bg-gray-900 border-b border-gray-800 p-3 flex flex-col gap-2 z-50">
        <div className="flex justify-between items-center">
          ...
          <div className="text-xs font-bold text-gray-500 flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
            Duración: <span className="font-mono text-emerald-400">{formatoTiempoDinamico(tiempoTotal)}</span>
          </div>

          <div className="flex items-center gap-1">
            <button onClick={() => setTiempoDescanso(t => Math.max(0, t - 30))} className="w-8 h-8 bg-gray-800 text-gray-400 rounded-lg font-bold hover:bg-gray-700 active:scale-95">-30</button>
            <div className={`text-lg font-mono font-bold px-2 py-1 rounded-lg text-center min-w-[70px] ${descansoActivo ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/50' : 'text-gray-500'}`}>
              {formatoTiempoDinamico(tiempoDescanso)}
            </div>
            <button onClick={() => setTiempoDescanso(t => t + 30)} className="w-8 h-8 bg-gray-800 text-gray-400 rounded-lg font-bold hover:bg-gray-700 active:scale-95">+30</button>
          </div>
        </div>
      </header>
